...
<script>
		...
		target.summernote({//JSON형태로 각각의 설정이 있음, 키값으로 정해져있음, CSS는 불가능
			...
			callbacks : {
				...
				onMediaDelete : function(target){
					//alert(target.attr("src"));	//이미지 태그 - jquery
					//alert(target[0].src);			//순수 javascript객체에 접근, 0번째에 들어있음
					/* var splitSrc = target[0].src.split("=");
					var fileName = splitSrc[splitSrc.length - 1];
					alert(fileName); */
					deleteFile(target[0].src);		//target, target[0].src, fileName 등 무엇을 넘겨도 내용의 차이는 별로 없음
				}
			}
		});
	}
	...
	function deleteFile(src){
		...
		$.ajax({
			...
			data : JSON.stringify(fileData),	//json객체를 String형태로 변환
			type : "post",
