   ...
   <!-- Main content -->
    ...
    <script>
    	window.onload = function(){	//js를 아직 못 읽기 때문에 window.onload를 해줌
    		...
    		/* $('a[name="attachedFile"] > button').click(function(event){
    			return false;
    			//이건 이벤트 핸들러에서만 사용할 수 있다. 펑션콜에서는 사용이 안된다.
    			//버튼이 a태그 안에 있지만, 버튼이 받은 이벤트를 a태그에 보내지 않게 된다.
    			//하지만 펑션콜을 하면 return false;가 dispatcher에 도달하지 않게 된다.
    			//그러므로 펑션콜이 존재하는 경우에는 펑션콜 뒤에 return false를 붙여줘야 한다.
    		}); */
    	}
    </script>
    ...
    <script>
    	...
    	function removeFile_go(className){
    		...
    		//$('li.' + className).remove();
    		//새로고침하면 삭제된게 다시 나타남.
    		//수정버튼을 누르면 그때서야 DB와 local HDD에서 삭제된어야 한다.
    		var li = $('li.' + className);
    		...
    		//removeArr.push(li.attr("target-ano"));	
    		
    		//파일삭제 hidden input 생성
