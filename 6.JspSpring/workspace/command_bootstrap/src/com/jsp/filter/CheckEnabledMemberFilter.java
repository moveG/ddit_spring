...
public class CheckEnabledMemberFilter implements Filter {
	...
	public void init(FilterConfig fConfig) throws ServletException {
		...
		StringTokenizer token = new StringTokenizer(checkUrlsParam, ",", false);	//false: 기준점(콤마)은 제외하고 저장
		
		while(token.hasMoreTokens()) {
			checkUrlsSet.add(token.nextToken().trim());
		}
	}
	...
	public void doFilter(ServletRequest request, 
		...
		if(checkUrlsSet.contains(endPoint)) {	//web.xml에서 가져온 init-param에 endPoint와 동인한 것이 있는지 확인하여 true/false를 반환
			...
			chain.doFilter(request, response);	//regist, modify, remove가 아닌 경우
		}
	}
